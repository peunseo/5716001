...
Node* doBinaryBatch(Node* root) {
    int compareCount = 0, searchCount = 0;  // 비교횟수, 탐색횟수 초기화
    for (int i = 0; i < 2000; i++) {
        int A = rand() % 3;     // A는 0~2
        int B = rand() % 1000;  // B는 0~999
        if (A == 0) root = avlInsert(root, B);  // A가 0이면 B를 추가
...
// 215번째 줄에 compareCount = allCount = 0;에서 초기화 값이 compareCount = searchCount = 0; 게 아닌가 여쭙습니다 ..
// allCount는 219번째 줄에서 값이 최신화된다고 생각합니다 ..
// 그래서 맞추기위해 bin에서 따로 초기화하긴 했습니당 ..
